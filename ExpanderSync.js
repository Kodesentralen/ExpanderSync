...
client.interceptors.response.use(
	...
	(err) => {
		...
		if (
			...
			error.headers["www-authenticate"] === "Negotiate, NTLM" &&
			...
			error.status === 401 &&
			error.headers["www-authenticate"] &&
			error.headers["www-authenticate"].substring(0, 4) === "NTLM"
		) {
...
const sendType3Message = (token) => {
	var type2msg = ntlm.parseType2Message(token, (err) => {
		console.log(err);
	});
...
const scrambleSecrets = []; // Array of path's to scramble secrets in, e.g. "Configuration.crmscript"
...
const dbToDisk = {
	extra_menus: {
		fields:
			"id,screen,label,url,target,icon_url,extra_info,order_pos,flags,base_program,extra_table,group_id",
	},
	ejscript: {
		cleanFolder: "ejscript",
		fields:
			"id,description,include_id,hierarchy_id.fullname,registered,updated,body",
	...
	screen_definition: {
		...
		json: "id:Integer,name:String,id_string:String,folder:String,screen_key:String,load_script_body:String,load_post_cgi_script_body:String,load_final_script_body:String,creation_script:String,warn_on_navigate:Boolean,description:String,autosave:Boolean",
		...
		children: {
			screen_definition_element: {
				...
				json: "id:Integer,name:String,screen_definition:String,folder:String,element_type:Integer,description:String,creation_script:String,order_pos:integer,base_table:String,hide:Boolean",
				jsonFile: true,
				scriptFiles: ["creation_script"],
				folderField: "screen_definition.hierarchy_id.fullname",
				filename:
					"screen_definition/${folder}/${screen_definition}/screen_definition_element/${id}-${name}",
			...
			screen_definition_action: {
				fields:
					"id,button,ejscript_body,screen_definition.name,screen_definition.hierarchy_id.fullname",
				json: "id:Integer,button:String,ejscript_body:String,screen_definition:String,folder:String",
				scriptFiles: ["ejscript_body"],
				folderField: "screen_definition.hierarchy_id.fullname",
				filename:
					"screen_definition/${folder}/${screen_definition}/screen_definition_action/${button}",
			},
	...
	extra_tables: {
		...
		json: "id:Integer,table_name:String,name:String,search_header:String,view_entry_header:String,new_entry_header:String,edit_entry_header:String,folder:String,display_field:String,description:String",
		jsonFile: true,
		...
		children: {
			extra_fields: {
				fields:
					"id,domain,extra_table,target_extra_table.table_name,field_name,name,default_value,type,flags,params,description",
				json: "id:Integer,domain:Integer,extra_table:Integer,target_extra_table:String,field_name:String,name:String,default_value:String,type:Integer,flags:Integer,params:String,description:String",
...
async function putElement(elementInfo, element, mtime) {
	...
	const obj = {};
	for (const updateField of elementInfo.updateFields)
		obj[updateField] = element[updateField];
	for (const scriptFile of elementInfo.scriptFiles)
		obj[scriptFile] = fs.readFileSync(
			targetPath + element.filename + "." + scriptFile + ".crmscript",
			"utf8",
		);

	if (elementInfo.hasRegisteredUpdated)
		obj.updated = buildDateTimeString(mtime);
	if (noPut === false) {
		const response = await axios.post(
			endpoint + "&table=" + elementInfo.table,
			obj,
		);
	}
}
...
function fileShouldBeIgnored(filename) {
	...
	for (const path of ignore) {
		if (
			path.length > 1 &&
			path[0] === "*" &&
			filename.indexOf(path.substring(1)) >= 0
		)
			result = true;
	...
	if (includeOnly.length > 0) {
		...
		for (const path of includeOnly) {
			if (filename.indexOf(path) >= 0) result = false;
		}
		printOutput(
			3,
			"includeOnly specified: " + filename + ", should be ignored: " + result,
		);
	}
...
function fileShouldBeScrambled(filename) {
	let result = false;
	for (const path of scrambleSecrets) {
		if (path.length > 1 && filename.indexOf(path.substring(1)) >= 0) {
			result = true;
		}
	}
	return result;
...
function createFolderAndFile(filename, data, mtime) {
	if (data) {
		if (!fileShouldBeIgnored(filename)) {
			...
			if (
				ignoreJSONFiles === false ||
				path.extname(filename).toLowerCase() !== ".json"
			) {
				let existingData = null;

				if (fileShouldBeScrambled(filename)) {
...
function getAttributesExceptScriptfiles(elementInfo, element) {
	...
	for (const key in element) {
		if (!elementInfo.scriptFiles || elementInfo.scriptFiles.indexOf(key) < 0)
			obj[key] = element[key];
	}
...
function appendToNavigationFile(filename, element) {
	if (!(filename in memoryFile)) memoryFile[filename] = { data: "" };
...
async function getElement(elementInfo, element) {
	...
	if (elementInfo.table === "item_config" && element.item_name === "body") {
		createFolderAndFile(
			targetPath + element.filename + ".body.crmscript",
			element.item_value,
			element.mtime,
		);
		return;
...
function getMtimeForElement(elementInfo, element) {
	const files = [];
	if (elementInfo.jsonFile === true)
		files.push(targetPath + element.filename + ".json");
	if (elementInfo.scriptFiles)
		for (const scriptFile of elementInfo.scriptFiles)
			files.push(
				targetPath + element.filename + "." + scriptFile + ".crmscript",
...
async function checkElement(elementInfo, element, method) {
	...
	if (method === "status") {
		if (element.mtime === null)
			printFileStatus(element.filename, "     no date in database");
		...
		if (element.mtime === null)
			printFileStatus(
				element.filename,
				"     no date in database, cannot sync",
			);
		else if (element.mtime > mtime || mtime === null) {
			printFileStatus(element.filename, " <== database");
			await getElement(elementInfo, element);
		...
		for (const element of elements) {
			if (verboseLevel >= 1)
				process.stdout.write(
					"\r - " + elementInfo.table + ": " + counter + "/" + elements.length,
				);
			counter++;
			...
			if (element.filename) {
				...
				if (
					elementInfo.table === "screen_definition_element" &&
					((screenElementNameOnlyFromId > 0 &&
						element.id >= screenElementNameOnlyFromId) ||
			...
			} else if (elementInfo.filenameLookup)
				element.filename =
					filenamesMap[evaluateTemplate(elementInfo.filenameLookup, element)];
			else element.filename = element.folder + "/" + element.name;
			if (elementInfo.hasRegisteredUpdated === true)
				element.mtime = parseDateTimeString(
					element.updated ? element.updated : element.registered,
				);
			...
			if (element.filename) {
				...
				for (let i = 0; i < escapeCharacters.length; i++) {
					element.filename = element.filename.myReplaceAll(
						escapeCharacters[i],
						escapeWith,
					);
				}
...
async function doTable(
	...
	if (elementInfo.where) {
		for (const where of elementInfo.where)
			url = addWhereClause(url, where.field, where.operator, where.value);
	}
	...
	// Sub tables
	for (const child in elementInfo.children)
		await doTable(elementInfo.children[child], child, method, pathStartsWith);
...
function calculateIdRange(idRange, returnAsArray) {
	...
	for (let part of parts) {
		...
		if (start > end)
			throw "calculateIdRange: start cannot be larger than end: " + part;
		if (end - start > 500) throw "calculateIdRange: range is too long: " + part;

		if (remove) {
			for (let i = start; i <= end; i++) ids = ids.filter((v) => v !== i);
		} else {
...
function parseArgs(myArgs) {
	for (var i = 0; i < myArgs.length; i++) {
		if (myArgs[i] === "--configFile" && i + 1 < myArgs.length)
			loadConfigFile(myArgs[++i]);
		else if (myArgs[i] === "-e" && i + 1 < myArgs.length)
			endpoint = myArgs[++i];
		else if (myArgs[i] === "-m" && i + 1 < myArgs.length) method = myArgs[++i];
		else if (myArgs[i] === "-p" && i + 1 < myArgs.length)
			...
			ignore.push(myArgs[++i].replace("|", " "));
		else if (myArgs[i] === "--scrambleSecrets" && i + 1 < myArgs.length)
			scrambleSecrets.push(myArgs[++i].replace("|", " "));
		else if (myArgs[i] === "--includeOnly" && i + 1 < myArgs.length)
			includeOnly.push(myArgs[++i].replace("|", " "));
		...
		else if (myArgs[i] === "--ejscriptCorrectFolder")
			dbToDisk.ejscript.filename = "ejscript/${folder}/${name}"; // No separate subfolder per script
		else if (myArgs[i] === "--escapeCharacters" && i + 1 < myArgs.length)
			escapeCharacters = myArgs[++i];
		else if (myArgs[i] === "--escapeWith" && i + 1 < myArgs.length)
			escapeWith = myArgs[++i];
		else if (myArgs[i] === "--dumpTable" && i + 3 < myArgs.length)
			addDumpTable(myArgs[++i], myArgs[++i], myArgs[++i]);
		else if (
			myArgs[i] === "--screenElementNameOnlyFromId" &&
			i + 1 < myArgs.length
		)
			screenElementNameOnlyFromId = parseInt(myArgs[++i]);
		else if (
			myArgs[i] === "--screenElementNameOnlyIds" &&
			i + 1 < myArgs.length
		)
			screenElementNameOnlyIds = calculateIdRange(myArgs[++i], true);
...
async function main() {
	...
	if (noPut)
		printOutput(0, "noPut is true, no files will be pushed to server!");
	...
	if (method === "sync" && verboseLevel < 2) {
		...
		else printOutput(0, "No sent files");
		if (syncReceivedFiles.length > 0)
			printOutput(
				0,
				"Received files: \r\n- " + syncReceivedFiles.join("\r\n- ") + "\r\n",
			);
		else printOutput(0, "No received files");
	}
